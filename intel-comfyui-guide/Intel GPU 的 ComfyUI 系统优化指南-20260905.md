...
> 本指南配套同目录下的安装文件包使用，版本：**20260905**

**📌 本版本更新说明（20260905）**：

- ⚠️ **本版为 torch 2.14 栈 / B 系列（BMG）和 A770(dg2) 双线，请区分不同的文件夹。**
- **omni_xpu_kernel**：0.2.0b2+torch213.bmg → **0.2.0b2+torch214.bmg**——**PyTorch 2.14 重建版**（同步上游至 #675；Windows 编译标准升 **C++20**——torch 2.14 headers 硬性要求）。CUTE FMHA + Sol-Attn 四算子（d128 / Wan cross / H3 VAE / sol_attn）随 bmg 内核保持全启用
- **comfy-kitchen**：**架构切换**——0.2.31.post1 整包 → **xiangyuT provider 架构**（`comfy_kitchen_xpu_runtime-0.2.31`）。官方 comfy-kitchen 0.2.31 由 ComfyUI requirements 自动安装，XPU 实现走 **OmniXPU runtime provider 路由**（与官方版解耦，**升级 ComfyUI 内核不再互相覆盖**）
- **ComfyUI-OmniXPU**：沿用 20260901 版（上游无更新；该版已含 runtime bootstrap + provider 契约 comfy_kitchen/comfy_aimdo）
- **ComfyUI-GGUF-XPU**：**20260905 新版**（`ComfyUI-GGUF-XPU-20260905.zip`）——新增 **Qwen3-VL 视觉塔（mmproj）加载映射**（`loader.py`，对应上游 PR [city96/ComfyUI-GGUF#478](https://github.com/city96/ComfyUI-GGUF/pull/478)），修复 Krea2 工作流加载 GGUF 版 Qwen3-VL CLIP 时报 `30720 features but got 2560` 的问题；qwen2vl/llama/t5 等原加载路径零回归
- **ComfyUI-SolAttn / comfy-aimdo**：沿用 20260901 版（无变化；aimdo 维持 PR#17）
- **配套内核**：ComfyUI 0.34.x（0.34.5 实测）+ **PyTorch 2.14.0+xpu**（XPU build 20260100 = oneAPI 2026.1）
...
#### 2.1 A770 显卡安装（社区手搓内核，dg2）

A 系列官方原版不支持，使用社区维护者 Blackwood416 的 dg2 内核。同架构A750可能会报错。

> ✅ **本版（20260905）`A770(dg2)/` 目录随包提供**——torch 2.14 栈社区 dg2 内核，安装文件即下方所列。
...
解压 `A770(dg2)/ComfyUI-OmniXPU.A770(dg2)-20260905.zip` 到 `ComfyUI\custom_nodes\`（解压后即得 `ComfyUI-OmniXPU` 文件夹）  
...
上面 A770 与 B 系列是两套手动变量（`esimd` vs `cute`），分享或换机时容易配错。更省心的做法是把它合并成**一份通用启动 bat**：启动开头先检测显卡型号，按结果自动设置对应的一组变量，一套文件通吃：
...
- **各分支动作**：bmg → 设上面 B 系列那组（`OMNI_ATTN_BACKEND=cute` 等）；dg2 → 设 A770 那组（`esimd` 等）；兜底 → 设 `OMNI_ATTN_BACKEND=torch`（官方 PyTorch SDPA、不打 attention 补丁，最保守、不会出错）。
...
### 5. ComfyUI Kitchen XPU 安装（选装；20260905 起为 provider 新架构）
> 🔄 **后续升级**：从这个项目找更新包 → https://github.com/xiangyuT/comfy-kitchen-xpu

> ⚠️ **新架构说明（20260905）**：kitchen 改走 **xiangyuT provider 架构**——官方 `comfy-kitchen` 0.2.31 由 ComfyUI `requirements.txt` 自动安装（官方 PyPI 版，不含 xpu 后端），XPU 实现打包为独立 **provider 发行 `comfy_kitchen_xpu_runtime`**，由 ComfyUI-OmniXPU 启动时（prestartup）按契约路由激活。**不再互相覆盖，与 ComfyUI 依赖管理兼容**。

> kitchen 不分显卡系列，provider wheel 一个通用（在本包根目录）；官方 comfy-kitchen 由 ComfyUI 自动装好，无需手动处理。

**安装方法（装 provider wheel）**

```text
python -m pip install comfy_kitchen_xpu_runtime-0.2.31-py3-none-any.whl
```

**生效前提**：ComfyUI-OmniXPU 为 20260901+ 版（含 `runtime_bootstrap.py`，本包已带）；安装后 **重启 ComfyUI**，启动日志出现 provider active（bootstrap 路由）即生效。


**ComfyUI 内核升级后还会失效吗？——不会（新架构已解决）**

旧版（0.2.31.post1 整包）会被 ComfyUI 更新器按 `requirements.txt` 重装官方版覆盖，xpu 后端丢失。**provider 架构下官方版与 XPU provider 各司其职**：官方 comfy-kitchen 由 pip 管理、provider wheel 常驻 site-packages，升级 ComfyUI 内核不再影响 XPU 接入。

**若升级后发现 xpu 路由未生效**：确认 provider 已装（`pip show comfy-kitchen-xpu-runtime`）+ OmniXPU 为 20260901+ 版 + 重启 ComfyUI。
...
- 安装文件（本包根目录）：`ComfyUI-GGUF-XPU-20260905.zip`（本版新增 Qwen3-VL 视觉塔支持，Krea2 工作流加载 GGUF 版 Qwen3-VL CLIP 必需）
...
**② kitchen xpu provider 生效**（20260905 起为 provider 架构）：
...
```
[OmniXPU] omni_xpu_kernel 0.2.0b2+torch214.bmg - available: norm, rotary, linear_fp8, int8
...
| kitchen xpu 路由没生效 | provider 未装 / OmniXPU 太旧 / 未重启 | 装本包 provider wheel + OmniXPU 20260901+ + 重启 ComfyUI |
...
### 实测数据（Arc B580 / Windows；数据基于 torch 2.13 栈测得，torch 2.14 栈待复测）
...
A：架构不同（DG2 vs BMG），内核 wheel 不通用。A 系列（社区内核）与 B 系列（官方内核）分目录；**本版（20260905）为 torch 2.14 栈，`B系列(bmg)/` 与 `A770(dg2)/` 随包各含一份**。kitchen/节点/aimdo 通用不分卡。


**Q2：需要安装 oneAPI 吗？**
A：**不需要**。oneAPI 只是编译内核时的工具；运行时的 DLL 依赖（sycl9 等）由 torch 2.14 环境自带。前提是 torch 必须是 2.14.0+xpu（与内核 wheel 配对）。
...
| comfy-kitchen | ❌ **不会**（20260905 起 provider 架构，与官方解耦） | 重装本包 provider wheel | 否（纯 python） |
...
### 6.3 comfy-kitchen：provider 架构下不再被覆盖（20260905 起）

- **旧版问题**（≤20260901 的 0.2.31.post1 整包）：`requirements.txt` 固定官方 kitchen → 升级时被官方纯版覆盖（xpu 后端丢失）
- **新版机制**（20260905 provider 架构）：官方 comfy-kitchen 由 pip 管理、XPU provider（`comfy_kitchen_xpu_runtime`）常驻 site-packages，二者解耦 → 升级 ComfyUI 内核不再影响 XPU 接入
- **检查**（provider 是否就绪）：
...
- **验证 xpu 路由**：重启 ComfyUI 看启动日志 provider active；或先模拟 prestartup 激活 provider（OmniXPU 的 `runtime_bootstrap.py`）再独立验证：
...
- **恢复**：provider wheel 丢失 → 重装本包 `comfy_kitchen_xpu_runtime-0.2.31-py3-none-any.whl`（`pip install --force-reinstall --no-deps`），再重启 ComfyUI
...
2. **检查 kitchen provider**（必做）→ `pip show comfy-kitchen-xpu-runtime` + OmniXPU provider active → 无则重装本包 provider wheel
...
```
IntelGPU-ComfyUI-系统优化指南-20260905/
├── 本指南.md                      ← 你现在看的（torch 2.14 栈 / B 系列(bmg) 与 A770(dg2) 双线）
├── A770(dg2)/                     ← A 系列（A770 等 DG2）专用（社区内核）
│   ├── omni_xpu_kernel-0.2.0b2+torch214.dg2-cp313-....whl   ← 社区 dg2 内核（torch 2.14 重建）
│   └── ComfyUI-OmniXPU.A770(dg2)-20260905.zip               ← 社区版节点（Blackwood416）
├── B系列(bmg)/                    ← B 系列（B580/B60/B70）专用
│   ├── omni_xpu_kernel-0.2.0b2+torch214.bmg-cp313-....whl   ← 含 Windows CUTE + Sol-Attn（torch 2.14 重建）
│   ├── ComfyUI-OmniXPU.bmg-20260901.zip                     ← 官方版（沿用；含 runtime bootstrap）
│   └── ComfyUI-SolAttn-20260901.zip                         ← Patch Sol-Attn 稀疏注意力节点（沿用）
├── comfy_kitchen_xpu_runtime-0.2.31-py3-none-any.whl  ← kitchen XPU provider（新架构；官方 comfy-kitchen 由 ComfyUI 自动装）
├── ComfyUI-GGUF-XPU-20260905.zip   ← GGUF 加速节点（新增 Qwen3-VL/Krea2 视觉塔补丁，选装）
└── comfy_aimdo_xpu_win_PR17.zip    ← aimdo（沿用，选装测试版：PR#17）
