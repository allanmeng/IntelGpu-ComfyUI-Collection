...
> 本指南配套同目录下的安装文件包使用，版本：**2026-08-14**
...
> ⚠️ **重要提醒**：本包内置的 aimdo 是 **PR#4 测试版**（上游 commit `323c0e0`，未合并稳定版），**部分设备上存在已知问题**：
> - 问题 A：模型总需求接近/超过显存时，显存无上限增长、驱逐不触发 → **采样 step 1 卡死**
> - 问题 B：与 omni_xpu_kernel 的 esimd 内核存在全局 hook 冲突 → **开启后 norm 每次调用 ~200ms 卡死**
>
> → **普通用户建议暂不启用 aimdo**；确需大模型显存管理，请关注上游 issue #3 的修复进展，等待稳定版。
...
解压 `B系列(bmg)/ComfyUI-OmniXPU.bmg-20260814.zip` 到 `ComfyUI\custom_nodes\`（解压后即得 `ComfyUI-OmniXPU` 文件夹）  
重启 ComfyUI 后自动加载（日志出现 `[OmniXPU]` 即成功）。
...
> ⚠️ **版本对齐提示**：本包提供的 kitchen 是 **0.2.31**，与 **ComfyUI 0.33.1** 配套对齐（基于 0.2.28 XPU fork 手动合并官方 0.2.31 并修复官方 cuda 导入问题制作）。**每次升级 ComfyUI 内核后，kitchen 都可能需要重新合并上游代码 + 打 XPU patch + 重新安装**，维护成本较高——请评估后再决定是否安装。
...
```text
python -m pip install comfy_kitchen-0.2.31-py3-none-any.whl
```
...
> ⚠️ **当前版本是 PR#4 测试构建**（对应上游 `323c0e0`），含已知问题（见第二部分）。**普通用户建议跳过本节**，等待稳定版。


**安装文件的下载与部署**

把本包 `comfy_aimdo_xpu_win_pr4/` 整个文件夹放到 **ComfyUI-aki 根目录**（与 `python` 文件夹同级），**双击运行 `deploy.bat`**：
...
- 安装文件（本包根目录）：`ComfyUI-GGUF-XPU-20260814.zip`
...
| 启动日志 `backends` 里没有 xpu | kitchen 被官方版覆盖（升级后） | 重装 `comfy_kitchen-0.2.31` wheel |
...
| 开启 aimdo 后卡死 | PR#4 测试版已知问题 | **关闭 aimdo**（去掉 `--enable-dynamic-vram`） |
...
**Q4：aimdo 为什么是测试版？**
A：上游 PR#4 尚未合并稳定（显存驱逐策略 + 与内核 hook 冲突两个问题未闭环，见 issue #3）。正式版发布后再更新本包。
...
```
IntelGPU-ComfyUI-优化指南-20260814/
├── 本指南.md                      ← 你现在看的
...
│   ├── omni_xpu_kernel-0.2.0b1+torch213.bmg-cp313-....whl
│   └── ComfyUI-OmniXPU.bmg-20260814.zip
├── comfy_kitchen-0.2.31-py3-none-any.whl   ← kitchen（选装谨慎；与 ComfyUI 0.33.1 对齐）
├── ComfyUI-GGUF-XPU-20260814.zip   ← GGUF 加速节点（选装，GGUF 用户强烈建议）
└── comfy_aimdo_xpu_win_pr4/       ← aimdo（选装，PR#4 测试版）
    ├── deploy.bat                 ← 一键部署脚本
